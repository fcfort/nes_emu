...
/**
 * Controls interaction between cpu/nes and command line input. This will be used for debugging the
 * cpu core.
 *
...
public class ConsoleDebugger {
  ...
  public void usage() {
    System.out.println("Welcome to the NES debugger." + " Enter ? for help.");
  ...
  public String status() {
    String cpuStatus =
        String.format(
            "CPU - PC: %s SP: %s A: %s X: %s Y: %s Status: S %s V %s D %s I %s Z %s C %s",
    ...
    String ppuStatus =
        String.format(
            "PPU - Cycle: %d Coarse X: %d Fine X: %d Y: %d Hori: %d Vert: %d T: %s V: %s CTRL: %s MASK: %s STATUS: %s",
  ...
  public String patterns() {
     ...
     */
    for (short addr = PPUMemory.PATTERN_TABLE_0_LOC;
        addr < PPUMemory.PATTERN_TABLE_1_LOC + PPUMemory.PATTERN_TABLE_SIZE;
        addr++) {
      ...
      logger.debug(
          "Got rowNumber {}, colNumber {}, handIndex {}, x {}, y {}, tile offset {} with value {} and address {}",
          new Object[] {
            rowNumber,
            colNumber,
            handIndex,
      ...
      for (int i = 0; i < 8; i++) {
        ...
        logger.debug(
            "Copying bit value {} to handIndex {}, x {}, y {}",
            new Object[] {bit, handIndex, x + i, y});
